...
const { buildStageSubLabels, formatEstimatedDelivery } = require('./stage-labels');
...
function normalizeFromApi(payload, fallbackNumber) {
  ...
  const stageSubLabels = buildStageSubLabels(rawEvents, activeStageIndex, d.estimatedDelivery);
  const estimatedDeliveryLabel = formatEstimatedDelivery(d.estimatedDelivery);
